...
public class Loader {
    ...
    private ParseResult.Warning[] loadWarnings() {
        ...
        for (int i = 0; i < count; i++) {
            Nodes.WarningType type = Nodes.WARNING_TYPES[loadVarUInt() - 286];
            byte[] bytes = loadEmbeddedString();
            String message = new String(bytes, StandardCharsets.US_ASCII);
            Nodes.Location location = loadLocation();
    ...
    private Nodes.Node loadNode() {
        ...
        switch (type) {
            ...
            case 52:
                return new Nodes.FindPatternNode(startOffset, length, loadOptionalNode(), (Nodes.SplatNode) loadNode(), loadNodes(), loadNode());
            case 53:
